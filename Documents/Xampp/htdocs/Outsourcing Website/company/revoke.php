...
$emailc = mysqli_real_escape_string($mysqli, $companyEmail);
$sql0 = "SELECT * FROM revokes WHERE companyEmail = '$emailc' ";

$revokesCount = mysqli_num_rows(mysqli_query($mysqli, $sql0));

$sql = "SELECT * FROM jobapplications WHERE jobId = '$jobId' AND companyEmail = '$companyEmail'";

$rows = mysqli_fetch_row(mysqli_query($mysqli, $sql));

if(isset($rows[2])){
    $applicantEmail = mysqli_real_escape_string($mysqli, $rows[2]);
}
else{
    $rows[2] = "Job already revoked"; 
}
?>
    ...
    if (strlen($_POST['reason']) > 100){
        ...
        if(mysqli_num_rows(mysqli_query($mysqli, $sql))>0){
            ...
            if (mysqli_query($mysqli, $sql) && mysqli_query($mysqli, $sql2) && mysqli_query($mysqli, $sql3)){
                echo "<script> alert('An email will be send to applicant explaining the revoke, This job will be reopened for you to find another applicant'); </script>";
                ...
                $email -> addAddress($rows[2]);
                $email -> isHTML(true);

                $email -> Subject = 'Verification Email';
                $email -> Body = 'The Job ID '.$jobId.'Has been revoked by the outsourcing company due to this reason: <br>'.$revokeReason;
                ...
                $revokesCount = mysqli_num_rows(mysqli_query($mysqli, $sql0));
                if($revokesCount > 5){
                    $sql = "UPDATE companyDetails SET status = 'Blocked' WHERE companyEmail = '$emailc'";
  ...
  <body>
    ...
    <div class="card m-2 border-1">
        ...
        <div class="text-center">
            ...
            <h4>ApplicantEmail: <?php echo $rows[2] ?></h4>

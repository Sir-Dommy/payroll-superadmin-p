...
$email1 = $email; //$email1 to be used while sending email
...
if(isset($_POST['resendOTP']) || $_SESSION['prev'] == 'resetPassword'){
    ...
    $sql1 = "SELECT * FROM companydetails WHERE companyEmail = '$email'";
    if(mysqli_num_rows(mysqli_query($mysqli, $sql1)) == 1){
        $sql = "UPDATE companydetails SET OTP = '$OTP' WHERE companyEmail = '$email'";
        ...
        echo "<script>alert('You are not registered ensure that you have registered before resetting password')</script>";  
    }
...
if(isset($_POST['continue'])){
    ...
    if($password1 != $password2){
        ...
        $sql = "SELECT * FROM companyDetails WHERE companyEmail = '$email1'";

        $res = mysqli_query($mysqli, $sql);
        $rows = mysqli_fetch_row($res);

        if(mysqli_num_rows($res) < 1){
            echo "<script>alert('Ensure that you have registered before resetting password')</script>";
        }

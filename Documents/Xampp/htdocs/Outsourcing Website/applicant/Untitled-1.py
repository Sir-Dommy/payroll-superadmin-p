...
class Iff(Expr):     
    ...
    def satisfying_assignments(expr):     
        ...
        for assignment in dict_list:         
            if expr.evaluate(assignment):             
                ...
                yield sat   class KnowledgeBase(object):     
    def __init__(self):         
        self.facts = set()      
    def get_facts(self):         
        return self.facts      
    ...
    def ask(self, expr):         
        ...
        '''                           clauses = []         clauses = And(And(*self.facts).to_cnf(),Not(expr).to_cnf()).to_cnf()                  comparelist = self.get_atoms(clauses)         newlist = []         while (1):             for clause in clauses.hashable:                 for clause1 in clauses.hashable:                     if clause!=clause1:                         # clause is Or Object                         if (isinstance(clause, Or) == True):                             #  if clause1 is Or object                             if isinstance(clause1, Or) == True:                                 for x in clause.hashable:                                     for y in clause1.hashable:                                         if x == Not(y) or Not(x) == y:                                             list = self.get_atoms(clause)                                             list1 = self.get_atoms(clause1)                                             list.remove(x)                                             list1.remove(y)                                             lists = list + list1                                             clause = Or(*list)                                             clause1 = Or(*list1)                                             newlist.append(Or(*lists))                                             if len(list) == 0 or len(list1) == 0:                                                 return True                             else:                                 for x in clause.hashable:                                     if x == Not(clause1) or Not(clause1) == x:                                         tmp = self.get_atoms(clause)                                         tmp.remove(x)                                         clause = Or(*tmp)                                         tmp = self.get_atoms(clauses)                                         tmp.remove(clause1)                                         clauses = And(*tmp)                                         return True                         else:                             if isinstance(clause1, Or) == True:                                 for x in clause1.hashable:                                     if x == Not(clause) or clause == Not(x):                                         tmp = self.get_atoms(clause1)                                         tmp.remove(x)                                         clause1 = Or(*tmp)                                         tmp = self.get_atoms(clauses)                                         tmp.remove(clause)                                         clauses = And(*tmp)                                         return True                             else:                                 if clause == Not(clause1) or clause1 == Not(clause):                                     tmp = self.get_atoms(clauses)                                     tmp.remove(clause)                                     tmp.remove(clause1)                                     clauses = And(*tmp)                                     return True             if len(newlist)== 0:                 return False             comparelist = self.get_atoms(clauses)             if newlist in comparelist:                 return False             merge = newlist+comparelist             clauses = And(*merge)        a, b, c, d, e, f= map(Atom, "abcdef") print a == a print a==b print And(a,Not(b)) == And(Not(b),a) print Implies(a, Iff(b,c)) print And(a, Or(Not(b),c)) print Not(a).atom_names() print a.atom_names() expr = And(a, Implies(b, Iff(a,c))) print expr.atom_names() e = Implies(a,b) print e.evaluate({"a": False, "b": True}) print e.evaluate({"a": True, "b": False}) e = And(Not(a), Or(b,c)) print e.evaluate({"a": False, "b": False, "c": True}) ex = Implies(a,b) sat = satisfying_assignments(ex) print next(sat) print next(sat) print next(sat) e = Iff(Iff(a,b),c) print list(satisfying_assignments(e)) a, b, c, d, e, f= map(Atom, "abcdef") print "Or(And(a,b),And(c,d))" print Or(And(a,b),And(c,d)).to_cnf() print print "Or(And(a,b),c)" print Or(And(a,b),c).to_cnf() print print "Or(And(a,b),And(c,d),e)" print Or(And(a,b),And(c,d),e).to_cnf() print print "Or(And(a,Not(b)),And(Not(a),b))" print Or(And(a,Not(b)),And(Not(a),b)).to_cnf() print print "Or(Not(Or(b,c)),a).to_cnf()" print Or(Not(Or(b,c)),a).to_cnf() print print "Or(Not(a),Or(b,c)).to_cnf()" print Or(Not(a),Or(b,c)).to_cnf() print print "Implies(a,Or(b,c)).to_cnf()" print Implies(a,Or(b,c)).to_cnf() print print "Implies(Or(b,c),a).to_cnf()" print Implies(Or(b,c),a).to_cnf() print print "Iff(a,Or(b,c)).to_cnf()" print Iff(a,Or(b,c)).to_cnf() '''
############################################################ 
 # Section 2: Logic Puzzles 
############################################################  
# Puzzle 1  # Populate the knowledge base using statements of the form kb1.tell(...)  
kb1 = KnowledgeBase()  
...
mythical_query = mythical magical_query = magical horned_query = horned   
# Record your answers as True or False; if you wish to use the above queries, 
# they should not be run when this file is loaded 
is_mythical = False is_magical = True is_horned = True  
# Puzzle 2  # Write an Expr of the form And(...) encoding the constraints 
Ann = Atom("a") 
John = Atom("j") 
Mary = Atom("m")  
...
# Puzzle 3  # Populate the knowledge base using statements of the form kb3.tell(...) 
kb3 = KnowledgeBase()  
p1 = Atom("p1") 
e1 = Atom("e1") 
p2 = Atom("p2") 
...
puzzle_3_question = """ Sign 1 is true. Room1 contain the prize and room2 is empty.  """  
# Puzzle 4  
...
puzzle_4_question = """ And clauses were used between two suspects. """
...
feedback_question_2 = """
implementint the to_cnf(self) method was difficult.
Required writing long codes which required complicated logic.
"""

feedback_question_3 = """
Implementing the __eq__(self, other) method in section one.
It required writing less code and which was simple.
The assignment is fine, no parts should be changed
"